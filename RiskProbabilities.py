...
def allIn(attackingCountry, defendingCountry):
    return min(attackingCountry - 1, 3) if defendingCountry > 0 else 0

def advantage(attackingCountry, defendingCountry):
    return min(attackingCountry - 1, 3) if (attackingCountry - 1 > defendingCountry) and (defendingCountry > 0) else 0

p = RiskProbability()

x = reduce(lambda xs,x:
       xs + x, map(lambda r:
                   [r + [j] for j in range(1,7)], [[]]))
